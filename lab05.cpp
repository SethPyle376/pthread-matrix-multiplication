...
void* computeCell(void *p)
{
   int value = *((int *)p);
   int rows = matrix.numRowLHS;
   int columns = matrix.numColRHS;

   if (matrix.numRowLHS == matrix.numColRHS)
      rows++;
   
   int commonSize;

   commonSize = matrix.numColLHS;

   int targetRow = value / (rows - 1);
   int targetColumn = value % columns;

   int product = 0;
   for (int i = 0; i < commonSize; i++)
   {
      product += matrix.arrayLHS[(targetRow * matrix.numColLHS) + i] * matrix.arrayRHS[targetColumn + (i * columns)];
   }
   matrix.arrayDestination[value] = product;
}
...
bool multiply(MatrixMultiplication & m)
{
   ...
   int totalCells = numRowDes * numColDes;
   pthread_t threads[totalCells];
